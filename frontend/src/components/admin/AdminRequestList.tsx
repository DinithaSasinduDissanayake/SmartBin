...
// // // // import '../../styles/AdminRequestList.css';
...
// // // //         alert(`Error deleting admin request: ${errorData.message}`);
// // // //       }
// // // //     } catch (error) {
...
// // // //   const handleViewMore = (id: string, orderId: string) => {
// // // //     console.log('Navigating to request details with ID:', id);
// // // //     navigate(`/admin/request/${id}`, { state: { orderId } });
// // // //   };
...
// // // //               const orderId = String(index + 1).padStart(3, '0');
// // // //               console.log(`Request ${orderId}:`, req);
// // // //               return (
// // // //                 <tr key={req._id}>
// // // //                   <td>{orderId}</td>
// // // //                   <td>{req.name}</td>
...
// // // //                       onClick={() => handleViewMore(req._id, orderId)}
// // // //                     >
// // // //                       ViewMore
// // // //                     </button>
...
// // //       const isOrderIdMatch = orderId === query;
...
// // const AdminRequestList = forwardRef((props, ref) => {
// //   const [requests, setRequests] = useState<any[]>([]);
// //   const [filteredRequests, setFilteredRequests] = useState<any[]>([]);
// //   const [error, setError] = useState<string | null>(null);
// //   const [searchQuery, setSearchQuery] = useState<string>(''); // State for search input
...
// //   // Expose filteredRequests to parent via ref
...
//       const data = await response.json();
//       console.log('Fetched admin requests:', data);
//       // Assign orderId to each request based on original index
//       const requestsWithOrderId = data.map((req: any, index: number) => ({
//         ...req,
//         orderId: String(index + 1).padStart(3, '0'),
//       }));
//       setRequests(requestsWithOrderId);
//       setFilteredRequests(requestsWithOrderId); // Initialize filtered requests
//       setError(null);
//     } catch (err) {
//       console.error('Error fetching admin requests:', err);
//       setError('Failed to load requests. Please try again.');
...
//       const isNameMatch = req.name.toLowerCase().includes(query.toLowerCase());
//       const isEmailMatch = req.email.toLowerCase().includes(query.toLowerCase());
//       const isOrderIdMatch = req.orderId === query;
//       console.log(`Request ${req.orderId}: Name=${req.name}, Email=${req.email}, OrderIdMatch=${isOrderIdMatch}, NameMatch=${isNameMatch}, EmailMatch=${isEmailMatch}`);
//       return isNameMatch || isEmailMatch || isOrderIdMatch;
//     });
...
//                   <button
//                     className="view-more-btn"
//                     onClick={() => handleViewMore(req._id, req.orderId)}
//                   >
//                     ViewMore
//                   </button>
//                   <button className="delete-btn" onClick={() => handleDelete(req._id)}>
